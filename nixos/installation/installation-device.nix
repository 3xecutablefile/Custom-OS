...

{
  ...
  config = {
    ...
    # Use less privileged nebula user
    users.users.nebula = {
      isNormalUser = true;
    ...
    # Allow passwordless sudo from nebula user
    security.sudo = {
    ...
    services.getty.helpLine = ''
      The "nebula" and "root" accounts have empty passwords.

      To log in over ssh you must set a password for either "nebula" or "root"
      with `passwd` (prefix with `sudo` for "root"), or add your public key to
      /home/nebula/.ssh/authorized_keys or /root/.ssh/authorized_keys.
    ...
    nix.settings.trusted-users = [ "root" "nebula" ];
  };
}
