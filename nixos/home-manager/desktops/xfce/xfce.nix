...
with lib;
let
  cfg = config.nebula.desktops.xfce;
  bg-path = "/run/current-system/sw/share/backgrounds/nebula/${config.nebula.theme.background}"; # TODO: Fix Error
...
in
{
  options.nebula.desktops.xfce = {
    refined = mkEnableOption (mdDoc "Whether to enable nebulaOS's XFCE Refined desktop");
    picom = mkEnableOption (mdDoc "Whether to enable nebulaOS's XFCE Picom desktop");
  };
