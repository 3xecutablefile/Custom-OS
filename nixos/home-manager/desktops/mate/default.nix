{ lib, pkgs, config, ...}: let
  ...
  gtkTheme = "${config.nebula.theme-components.gtk-theme}";
  gtkIconTheme = "${config.nebula.theme-components.icon-theme}";
  gtkCursorTheme = "${config.nebula.theme-components.cursor-theme}";
  backgroundTheme = "${config.nebula.theme-components.background}";
in {
  config = lib.mkIf (config.nebula.desktopManager == "mate") {
    ...
    home-manager.users.${config.nebula.homeManagerUser} = { pkgs, ...}: {
      ...
      dconf.settings = {
          ...
          "org/mate/desktop/background" = {
              picture-filename = "/run/current-system/sw/share/backgrounds/nebula/"+backgroundTheme;
          };
      };
      ...
      dconf.settings = {
        "org/gnome/shell".favorite-apps = [ "nebula-welcome.desktop" "nebula-mimikatz.desktop" "nebula-powersploit.desktop" "seclists.desktop" "payloadsallthethings.desktop" "powershell.desktop" "shell.desktop" "codium.desktop" "firefox.desktop" "cyberchef.desktop" "fuzzdb.desktop" "securitywordlist.desktop" "autowordlists.desktop" ];
        ...
        # "org/mate/panel/objects/nebula-cyber-hub" = {
        #   launcher-location = "/run/current-system/sw/share/applications/org.nebulaos.CyberHub.desktop";
        #   object-type = "launcher";
        #   panel-right-stick = false;
        #   position = -1;
        #   toplevel-id = "top";
        ...
        "org/mate/panel/objects/nebula-welcome" = {
          launcher-location = "/run/current-system/sw/share/applications/nebula-welcome.desktop";
          object-type = "launcher";
          panel-right-stick = false;
          position = -1;
          toplevel-id = "top";
