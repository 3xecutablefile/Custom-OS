...
For nebula OS use case, by using Nix, you don't have to worry about syncing mirrors and hoping the mirror is up to date, or if the package is in the cache or it's not. Furthermore, also issues related to keyrings are not a concern anymore.
...
* by **nix-shell**: in my nebula I have FISH shell. When I add the nix-channel to unstable repo and I install nmap by `nix-shell -p nmap`, it creates a new environment with a BASH shell (not more FISH because the colors of my nebula prompt change to BASH one)  and it runs nmap in this environment. For the install I didnt have the need to install by sudo. In this environment nmap binary exists in a nix sandboxed dir (test by which nmap) Then, if I type `exit`, I go out from the env and I come back to FISH shell and nmap cannot be called anymore. Maybe it is a secure approach because we dont need to sudo. Note that the new environment is not an isolated or sandboxed environment. It just install and remove the declared package for on-off usage.
...
Once you created your `flake.nix` (look [nebula-nix](https://github.com/nebula-OS/nebula-nix) as example), you can deploy your configuration by running:
```
...
```
sudo nixos-rebuild switch --flake 'github:nebula-OS/nebula-nix#xfce'
```
...
Note that when you use Flakes for this purpose, you must not use or import modules related to `nixpkgs/nixos/nodules/installer/cd-dvd/<and-so-on>` because these ones are used for ISO creation. Remember also that the flake currently works with a user named `nebula` (defined in `home.nix` files).
...
```nix
  home-manager.users.nebula = { pkgs, ... }: {
    home.packages = [ pkgs.atool pkgs.httpie ];
    imports = [ "/home/nebula/nebula-nix/home.nix" ];
...
error: 1 dependencies of derivation '/nix/store/xxwnh88y46pkcylxgrf0p6q9zf3l1lwc-unit-home-manager-nebula.service.drv' failed to build
...
error: 1 dependencies of derivation '/nix/store/557gh4yh0cxl3f7vcpnpbpj00a42vk3g-nixos-system-nebulaos-24.05.20231219.54aac08.drv' failed to build
```
...
error: 1 dependencies of derivation '/nix/store/is3avirfqcb4a59ml9y2pc5ll66mn975-nixos-system-nebulaos-24.05.3640.56911ef3403a.drv' failed to build
```
